...
| **`BlackIce.BigLobby`** | Smooths >8-player matches: raises the room-*creator's* local PUN cap to match a large realm. **Not** needed to join — the server gates capacity, so unmodified clients join big realms fine. Game rendering above 8 is unverified. See its [README](BlackIce.BigLobby/README.md) and [`../docs/large-servers.md`](../docs/large-servers.md). | Optional |
