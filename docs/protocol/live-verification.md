...
|---|---|---|---|
| **DamagePacket.Damage type** | Read as a **big-endian float** at offset 0 (`PunRpcInfo`), and now also **written** there by `DamageData` (the single definition of the packet shared by the bots, the gameplay plugins, and the anti-cheat). The recon doc (`04-serialization.md`) calls it an **int32**. The two disagree; the code treats it as float — a capture validates `DamageData`. **NOTE (live capture):** in a *master-authoritative* co-op session no `TakeDamage` RPC is sent at all — the master resolves damage locally and broadcasts only `SetDying`. So `DamagePacket` only transits the wire for a **non-master** client taking damage (or PvP); capture it in that topology. | Capture a real `TakeDamage` RPC **as a non-master client** (second player, or bot-as-master); decode offset 0 both ways and see which yields a sane damage value. | [ac][codec] |
...
| ~~**Quaternion field order**~~ **CONFIRMED (x,y,z,w)** | Written/parsed as **(x,y,z,w)** big-endian — **verified** against a live SendSerialize(201) capture: a rotation logged as `x=0, y=0.953717, z=0, w=-0.3007057` with `eulerAngles.y=215°` (a Y-axis rotation has zero x/z, non-zero y/w). The earlier `(w,x,y,z)` concern is refuted; our order is correct. | ~~done~~ | [codec] |
...
|---|---|---|---|
| ~~**RPC shortcut index (key 5)**~~ **CAPTURED** | The server only resolves RPC **method names** (key 3); when a client sends the **shortcut byte index** (key 5) instead, the method was treated as unknown (`<shortcut rpc>`). The client's ordered `RpcList` is now captured — [`generated/rpc-shortcuts.csv`](generated/rpc-shortcuts.csv), 88 entries — so any `5:N` resolves to a name (verified live: `5:39`=`ReceiveChatMessage`). The server can now map shortcuts using this table. | ~~done~~ (re-dump after a game update) | [ac] |
...
| ~~**Death / respawn RPC**~~ **PLAYER SEQUENCE CAPTURED** | The kill model in the `killfeed` / `arena` plugins approximated death by summing `TakeDamage` toward an assumed max-HP. The real **player** death/respawn sequence is now captured live (pawn = a player PhotonView, here viewID 6001): **death** = `Shatter(59)` → `GoIntangible(27)` → `KilledPlayerRemote(32, arg=victimViewID)`; **respawn** = `TeleportImmediately(66, spawnPos:Vector3)` → `BecomeTangible(9)`. No `SetHealth` is broadcast (health is owner-local). Player death does **not** use `Die`/`SetDying` — those are the *enemy* path. **Remaining gap:** an environmental (lava) death carries the victim viewID but **no killer attribution**; PvP/enemy kill-credit (who killed whom) is unconfirmed — capture a player killed by another actor. | ~~done for the player path~~ — still: kill-*credit* for PvP/enemy deaths. | [mode][ac] |
| **Match size above ~8 players** | The server supports large rooms (thousands of actors structurally; per-realm `MaxPlayers`, `0` = unlimited) and enforces capacity itself, so unmodified clients **join** a >8 realm fine. What's unverified is whether the game's **rendering / HUD / netcode actually hold up** with >8 avatars in one match — it was built around ~8 with master-client authority. (`BlackIce.BigLobby` only aligns the room-creator's local cap; it doesn't make the game stable at size.) | Raise a server realm cap, join a match with >8 real/bot players (optionally with `BlackIce.BigLobby`), and observe: do all avatars render, does the HUD/scoreboard cope, does master-client authority stay stable? Note the size where it breaks. | [codec][mode] |
