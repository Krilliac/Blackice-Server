...
| **Match size above ~8 players** | The server supports large rooms (thousands of actors structurally; per-realm `MaxPlayers`, `0` = unlimited). The stock client is built around ~8-player matches; the client mod `BlackIce.BigLobby` raises the client's requested room capacity so a big realm can fill, but whether the game's **rendering / HUD / netcode actually hold up** in a single >8 match is **unverified**. | With `BlackIce.BigLobby` installed and a server realm cap raised, join a match with >8 real/bot players and observe: do all avatars render, does the HUD/scoreboard cope, does master-client authority stay stable? Note the size where it breaks. | [codec][mode] |
