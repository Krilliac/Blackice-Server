...
|---|---|---|---|
| **DamagePacket.Damage type** | Read as a **big-endian float** at offset 0 (`PunRpcInfo`). The recon doc (`04-serialization.md`) calls it an **int32**. The two disagree; the validators treat it as float. | Capture a real `TakeDamage` RPC; decode offset 0 both ways and see which yields a sane damage value. | [ac] |
